...
public class SharpProcessor extends AbstractProcessor {
    ...
    private void createDaggerComponent(SharpComponentModel model) {
        TypeSpec.Builder generatedComponentFileBuilder = TypeSpec.interfaceBuilder(model.getComponentName())
        ...
        if (model.scope.isSharpScoped()) {
            ...
            generatedComponentFileBuilder.addAnnotation(ClassName.get(model.packageString, model.getSharpScopeName()));
        } else {
            ClassName className = (ClassName) ClassName.get(model.scope.getScopeTypeMirror());
            generatedComponentFileBuilder.addAnnotation(className);
        }

        final ParameterSpec param = ParameterSpec.builder(model.annotatedClass, "thisClass").build();

        generatedComponentFileBuilder.addMethod(
        ...
        if (model.getComponentInstanceBindings().size() > 0) {
            generatedComponentFileBuilder.add
        }

        try {
            JavaFile.builder(model.packageString, generatedComponentFileBuilder.build())
